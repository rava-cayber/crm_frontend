...
const Teachers = () => {
  ...
  const fetchTeachers = async () => {
    try {
      const res = await fetch(`/api/v1/teachers?status=${showArchive ? 'inactive' : 'active'}`, {
        headers: { 
  ...
  const handleCreateTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setDrawerOpen(false);
        await fetchTeachers();
        setFormData({ full_name: '', email: '', password: '', phone: '+998', address: '', photo: null });
  ...
  const handleUpdateTeacher = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/v1/teachers/${selectedTeacher.id}`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${token}` },
        body: data
      });
    ...
    <div className="space-y-6 py-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-black text-[#1A2332] dark:text-white">O'qituvchilar</h1>
          <p className="text-xs text-slate-400 font-medium max-w-2xl">Ushbu sahifada siz o'qituvchilar ro'yxatini va ularning ma'lumotlarini topasiz.</p>
        </div>
        <Button
          variant="contained"
          startIcon={<AddIcon />}
      ...
      <div className="bg-white dark:bg-navy-elevated rounded-[32px] border border-slate-50 dark:border-navy-raised overflow-hidden shadow-sm">
        <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-50 dark:border-navy-raised">
          <div className="flex items-center gap-3">
            ...
            >
              <FilterListIcon fontSize="small" /> O'qituvchilar
            </button>
            <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50 dark:bg-navy-raised/50 border-b border-slate-100 dark:border-navy-raised">
              <tr>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest"><input type="checkbox" className="rounded" /></th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Nomi</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Telefon raqami</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Email</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Manzil</th>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-navy-raised">
              {teachersData.map((teacher) => (
                <tr key={teacher.id} className="hover:bg-slate-50/50 dark:hover:bg-navy-raised/30 transition-colors">
                  <td className="px-6 py-4"><input type="checkbox" className="rounded" /></td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <Avatar src={`http://localhost:3000/files/${teacher.photo}`} sx={{ width: 32, height: 32 }} />
                      <span className="text-xs font-bold text-[#1A2332] dark:text-white">{teacher.full_name}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-xs font-medium text-slate-500">{teacher.phone}</td>
                  <td className="px-6 py-4 text-xs font-medium text-slate-500">{teacher.email}</td>
                  <td className="px-6 py-4 text-xs font-medium text-slate-500">{teacher.address}</td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <IconButton size="small" className="text-slate-400 hover:text-primary"><VisibilityOutlinedIcon fontSize="small" /></IconButton>
                      <IconButton size="small" onClick={() => handleEditOpen(teacher)} className="text-slate-400 hover:text-blue-500"><EditOutlinedIcon fontSize="small" /></IconButton>
                      <IconButton size="small" onClick={() => handleDeleteTeacher(teacher.id)} className="text-slate-400 hover:text-red-500"><DeleteOutlinedIcon fontSize="small" /></IconButton>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Drawer anchor="right" open={drawerOpen} onClose={() => setDrawerOpen(false)}>
        <div className="w-[450px] h-full flex flex-col bg-white dark:bg-navy-elevated">
          <div className="p-8 border-b border-slate-50 dark:border-navy-raised flex items-center justify-between">
            <div>
              <h2 className="text-xl font-black text-[#1A2332] dark:text-white uppercase tracking-tight">O'qituvchi qo'shish</h2>
              <p className="text-xs text-slate-400 font-medium">Yangi o'qituvchi ma'lumotlarini kiriting.</p>
            </div>
          ...
          <form onSubmit={handleCreateTeacher} className="p-8 flex-1 overflow-y-auto space-y-5">
      ...
      <Drawer anchor="right" open={editDrawerOpen} onClose={() => setEditDrawerOpen(false)}>
        <div className="w-[450px] h-full flex flex-col bg-white dark:bg-navy-elevated">
          <div className="p-8 border-b border-slate-50 dark:border-navy-raised flex items-center justify-between">
            <div>
              <h2 className="text-xl font-black text-[#1A2332] dark:text-white uppercase tracking-tight">O'qituvchini tahrirlash</h2>
              <p className="text-xs text-slate-400 font-medium">O'qituvchi ma'lumotlarini yangilang.</p>
            </div>
          ...
          <form onSubmit={handleUpdateTeacher} className="p-8 flex-1 overflow-y-auto space-y-5">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Profil surati (ixtiyoriy)</label>
              <div className="border-2 border-dashed border-slate-100 dark:border-navy-raised rounded-2xl p-8 flex flex-col items-center justify-center text-center hover:bg-slate-50 dark:hover:bg-navy-raised transition-colors cursor-pointer relative">
                <input
                  type="file"
                  className="absolute inset-0 opacity-0 cursor-pointer"
                  onChange={e => setEditData({...editData, photo: e.target.files ? e.target.files[0] : null})}
                />
                <div className="w-12 h-12 bg-primary/10 text-primary rounded-2xl flex items-center justify-center mb-3">
                  <CloudUploadOutlinedIcon />
                </div>
